...
app.post('/api/auth/forgot-password', rateLimit(5, 15 * 60 * 1000), async (req, res) => {
  ...
  const user = db.prepare('SELECT id FROM users WHERE email = ? AND verified = 1').get(email);
  // Always return ok to prevent email enumeration
  if (!user) return res.json({ ok: true, message: 'If this email exists, a reset code was sent.' });
  ...
  try {
    const emailRes = await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({
        sender: { name: 'SMbot', email: BREVO_SENDER },
        to: [{ email }],
        subject: 'SMbot — Password Reset Code',
        htmlContent: `
          <div style="font-family:Arial,sans-serif;max-width:480px;margin:auto;padding:32px;background:#f9f9f9;border-radius:12px;">
            <h2 style="color:#059669;margin-bottom:8px;">🔐 Password Reset</h2>
            <p style="color:#444;">Use this code to reset your SMbot password:</p>
            <div style="font-size:36px;font-weight:bold;letter-spacing:8px;color:#059669;text-align:center;padding:24px 0;">${code}</div>
            <p style="color:#888;font-size:13px;">Expires in <strong>10 minutes</strong>. If you didn't request this, ignore this email.</p>
          </div>`
      })
    });
    if (!emailRes.ok) throw new Error('Brevo error');
    res.json({ ok: true, message: 'If this email exists, a reset code was sent.' });
  } catch(e) {
...
app.post('/api/auth/reset-password', rateLimit(5, 15 * 60 * 1000), async (req, res) => {
  ...
  const pending = db.prepare('SELECT * FROM pending_verifications WHERE email = ?').get(email);
  if (!pending)                    return res.status(400).json({ error: 'No reset request found. Please try again.' });
  if (Date.now() > pending.expires_at) {
    db.prepare('DELETE FROM pending_verifications WHERE email = ?').run(email);
